...
p.handleChunk = function(chunk) {
  ...
  if (csid == 2 || (typeid >= 1 && typeid <= 7)) {
    if (csid == 2 && typeid >= 1 && typeid <= 7) this.handleProtocol(chunk);
    else this.warn(util.format(
...
p.handleProtocol = function(chunk) {
  this.error('Protocol Control Message #' + chunk.typeid + ' not implemented');
}
