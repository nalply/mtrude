...
p.send = function(chunk) {
  if (chunk.msid != null) return send0(this.socket, chunk);
  if (chunk.typeid != null) return send1(this.socket, chunk);
  if (chunk.timestamp != null) return send2(this.socket, chunk);
...
function send1(socket, chunk) {
  throw new Error('unsupported chunk type 1');
}

function send2(socket, chunk) {
  throw new Error('unsupported chunk type 2');
}

function send3(socket, chunk) {
...
s.handshake0Server = function() { // RTMCSP 5.2 Handshake C0, S0 and S1
  ...
  if (version != 3) warn('RTMP version is not 3, but ' + version);
...
s.chunkType2 = function() { // RTMCSP 6.1.2.4 Chunk Type 2; additional chunk
  ...
  this.warn('chunkType2 timestamp 0x' + chunks.timestamp.toString(16));
